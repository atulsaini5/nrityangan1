...
export default function StudentAdmin({ accessCode, active }: { accessCode: string; active: boolean }) {
 ...
 const [statusFilter,setStatusFilter]=useState('all'),[classFilter,setClassFilter]=useState('all'),[yearFilter,setYearFilter]=useState('all'),[recitalFilter,setRecitalFilter]=useState('all');
 ...
 return <div>
  ...
  <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Student counts">{[['Students',students.filter(s=>s.kind==='student').length],['Active students',students.filter(s=>s.kind==='student'&&s.status==='active').length],['Guest artists',students.filter(s=>s.kind==='guest').length],[`${countYear||'Recital'} student performers`,students.filter(s=>s.kind==='student'&&s.recital_years?.includes(countYear)).length]].map(([label,count])=><div key={label} className="rounded-2xl bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold text-slate-900">{loaded?count:'—'}</p></div>)}</div>
  <p className="mb-5 text-xs text-slate-500">Student counts exclude guests and instructors. Recital counts include each student once. Students may be children or adults.</p>
  ...
  <div hidden={view!=='directory'}><div className="grid items-start gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
   <section aria-label="Student directory" className="rounded-2xl bg-white p-5 shadow-sm">
    ...
    <label className="mt-3 block text-sm">Show<select className={input} value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">Everyone</option><option value="student">Students</option><option value="guest">Guest artists</option><option value="instructor">Instructors</option><option value="review">Needs identity review</option></select></label>
    <div className="mt-3 flex flex-wrap gap-2">{[['all','Everyone'],['student','Students'],['guest','Guest artists']].map(([value,label])=><button type="button" key={value} aria-pressed={filter===value} onClick={()=>setFilter(value)} className={`rounded-full border px-3 py-2 text-xs font-semibold ${filter===value?'border-rose-700 bg-rose-50 text-rose-800':'border-slate-200'}`}>{label}</button>)}</div>
