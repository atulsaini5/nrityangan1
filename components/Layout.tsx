...
const useHashLocation = () => {
  ...
  const getLocation = () => indexedPaths.includes(window.location.pathname) || (/^\/(?:blog|recitals)(?:\/|$)/.test(window.location.pathname)) ? window.location.pathname : window.location.hash.replace(/^#/, '') || '/';
...
const Link = ({ to, children, className, onClick }: { to: string; children: React.ReactNode; className?: string; onClick?: () => void }) => {
  return (
    <a 
      href={to === '/' || to === '/blog' || to === '/recitals' ? to : `/#${to}`}
      className={className} 
      onClick={onClick}
    >
      {children}
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-800 bg-gray-50">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              ...
              <Link to="/recitals" className={location.pathname.startsWith('/recitals') ? 'text-rose-500 font-semibold' : 'text-gray-600 hover:text-rose-500'}>Annual Recitals</Link>
        ...
        {/* Mobile Menu Panel */}
        {isMenuOpen && (
          <div className="lg:hidden bg-white border-b border-gray-100 shadow-xl absolute w-full z-40">
            <div className="px-4 pt-2 pb-6 space-y-2 flex flex-col">
              ...
              <Link to="/recitals" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 text-base font-medium text-gray-700 hover:bg-rose-50 rounded-md">Annual Recitals</Link>
